...
//            currentWindow.addSubview(btn)
    ...
    public class func setConfig(config : Config) {
        ...
        loader.frame = CGRect(x: 50,y: 50,width: loader.config.size,height: loader.config.size)
    }
...
//        self.button?.frame = CGRect(x: loaderTitleMargin, y: loaderSpinnerMarginTop + loadingViewSize, width: self.frame.width - loaderTitleMargin*2, height: 42.0)
    }
